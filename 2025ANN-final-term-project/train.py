...
def plot_metrics(train_values, val_values, ylabel, save_path):
    ...
    print(f"Saved {ylabel} plot to {save_path}")
    plt.close()
...
def train_one_epoch(
    ...
    for src, tgt in pbar:
        ...
        mask = tgt_gold != criterion.ignore_index
        correct += ((pred == tgt_gold) & mask).sum().item()
        total += mask.sum().item()
...
def main() -> None:
    ...
    for epoch in range(1, cfg["train"]["epochs"] + 1):
     ...
     # 绘图
    plot_metrics(train_losses, val_losses, "Loss", save_dir / "loss_curve.png")
    plot_metrics(train_accuracies, val_accuracies, "Accuracy", save_dir / "acc_curve.png")
