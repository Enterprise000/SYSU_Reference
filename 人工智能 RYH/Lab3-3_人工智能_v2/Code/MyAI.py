...
class ChessAI(object):
    ...
    def get_next_step(self, chessboard: ChessBoard, depth):
        best_move = list()
        b_score = float('-inf')
        alpha = float('-inf')
        beta = float('inf')
        ...
        for chess in chessboard.get_chess():
            if chess.team == self.team:
                active_move = chessboard.get_put_down_position(chess)
                for new_row, new_col in active_move:
                    old_row = chess.row
                    old_col = chess.col
                    new_pos = chessboard.chessboard_map[new_row][new_col]
                    ...
                    score = self.minimax(chessboard, depth, alpha, beta, minimax)
                    ...
                    chessboard.chessboard_map[new_row][new_col] = new_pos
                    if score > b_score:
                        b_score = score
                        best_move.clear()
                        best_move.append(old_row)
                        best_move.append(old_col)
                        best_move.append(new_row)
                        best_move.append(new_col)
        return best_move

    def minimax(self, chessboard: ChessBoard, depth, alpha, beta, minimax):
        if depth == 0:
        ...
        if minimax:
            ...
            for chess in chessboard.get_chess():
                if chess.team == self.team:
                    max_active_move = chessboard.get_put_down_position(chess)
                    for new_row,new_col in max_active_move:
                        old_row, old_col = chess.row, chess.col
                        new_pos = chessboard.chessboard_map[new_row][new_col]
                        chessboard.chessboard_map[new_row][new_col] = chessboard.chessboard_map[old_row][old_col]
                        chessboard.chessboard_map[old_row][old_col] = None
                        chessboard.chessboard_map[new_row][new_col].update_position(new_row, new_col)
                        score = self.minimax(chessboard, depth - 1, alpha, beta, 0)
                        chessboard.chessboard_map[old_row][old_col] = chessboard.chessboard_map[new_row][new_col]
                        chessboard.chessboard_map[old_row][old_col].update_position(old_row, old_col)
                        chessboard.chessboard_map[new_row][new_col] = new_pos
            ...
            for chess in chessboard.get_chess():
                if chess.team != self.team:
                    min_active_move = chessboard.get_put_down_position(chess)
                    for new_row,new_col in min_active_move:
                        old_row = chess.row
                        old_col = chess.col
                        old_chess_in_new_pos = chessboard.chessboard_map[new_row][new_col]
                        chessboard.chessboard_map[new_row][new_col] = chessboard.chessboard_map[old_row][old_col]
                        chessboard.chessboard_map[old_row][old_col] = None
                        chessboard.chessboard_map[new_row][new_col].update_position(new_row, new_col)
                        score = self.minimax(chessboard, depth - 1, alpha, beta, 1)
                        chessboard.chessboard_map[old_row][old_col] = chessboard.chessboard_map[new_row][new_col]
                        chessboard.chessboard_map[old_row][old_col].update_position(old_row, old_col)
                        chessboard.chessboard_map[new_row][new_col] = old_chess_in_new_pos
