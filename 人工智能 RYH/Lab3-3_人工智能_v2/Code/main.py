...
from ChessAI import ChessAI as ChessAI
from MyAI import ChessAI as MyAI

def main():
# 第一部分：创建棋盘，画面，计时器，游戏，AI
    # 初始化pygame
    player = input("如果您想成为红方（先手），请输入1; 如果您想成为黑方（后手），请输入0")
    step = 0
    pygame.init()
    # 创建用来显示画面的对象（理解为相框）
    ...
    while True:
        ...
        if not game.show_win and not game.show_draw and game.AI_mode and game.get_player() == ai.team:
            ...
            if game.back_button.is_repeated():
                ...
                game.back_button.add_history(chessboard.get_chessboard_str_map())
                game.exchange()
        elif not game.show_win and not game.show_draw and game.AI_mode and game.get_player() == chessAi.team:
            step = step + 1
            if game.back_button.is_repeated():
                print("你是", game.user_team, "方,", game.get_player(), "方获胜")
                game.set_win(game.get_player())
                ...
                if step < 60:
                    ...
                    '''
              # AI预测下一步
                cur_row, cur_col, nxt_row, nxt_col = chessAi.get_next_step(chessboard,3)
              # 选择棋子
                ClickBox(screen, cur_row, cur_col)
              # 下棋子
                chessboard.move_chess(nxt_row, nxt_col)
