...
from config import INIT_TRAINER, SETUP_TRAINER, VERSION, CURRENT_FOLD, WEIGHT_PATH, WEIGHT_PATH_LIST, FOLD_NUM, \
    INIT_TRAINER_V1, INIT_TRAINER_V2
...
if __name__ == "__main__":
    ...
    parser.add_argument('-n',
                        '--net_name',
                        default="resnet34",
                        choices=["resnet18", "resnet34", "resnet50", "resnet101", "resnet152", "resnext50_32x4d",
    ...
    parser.add_argument('-bs',
                        ...
                        type=int)
    # 这行代码会解析命令行输入的参数，并将它们存储在args对象中。
    args = parser.parse_args()

    # 如果命令行参数中包含相关值，则会覆盖配置文件INIT_TRAINER中的默认设置。
    ...
    # 训练部分
    ###############################################
    # 会将数据分成多个折（例如 5 折或 10 折），然后每次训练时使用其中的一个折作为验证集，其他的作为训练集。
    # 训练会进行多次，每次验证不同的折。
    ...
    ###############################################
        ...
        # 单模型推理模式
        # 加载训练好的模型进行推理，并保存结果及特征。
        if args.mode == 'inf':
            # 初始化分类器并加载权重
            WEIGHT_PATH = './ckpt/v1.0_final/fold1/epoch=95-train_loss=0.00436-val_loss=1.04030-train_acc=0.99958-val_acc=0.79817.pth'
            classifier = VolumeClassifier(**INIT_TRAINER)
        ...
        #                 report_csv_file.to_csv(report_save_path)
            ...
            print("len(WEIGHT_PATH_LIST) =", len(WEIGHT_PATH_LIST))
            print("WEIGHT_PATH_LIST =", WEIGHT_PATH_LIST)
            INIT_TRAINER_LIST = [INIT_TRAINER, INIT_TRAINER_V2, INIT_TRAINER_V1]
            WEIGHT_PATH_LIST = [
                './ckpt/v1.0_task2-5_DO/fold1/epoch=69-train_loss=0.00711-val_loss=1.09770-train_acc=0.99937-val_acc=0.74812.pth',
                './ckpt/v1.0_final/fold1/epoch=95-train_loss=0.00436-val_loss=1.04030-train_acc=0.99958-val_acc=0.79817.pth',
                './ckpt/v1.0_task2-3_EM_m2/fold1/epoch=45-train_loss=0.00249-val_loss=1.14977-train_acc=1.00000-val_acc=0.76647.pth']
            results_pred_list = []
            results_true = None
            for fold in range(1, FOLD_NUM + 1):
                print('===================model %d===================' % (fold))
                classifier = VolumeClassifier(**(INIT_TRAINER_LIST[fold - 1]))
                checkpoint = torch.load(WEIGHT_PATH_LIST[fold - 1])
                classifier.net.load_state_dict(checkpoint['state_dict'])
                save_path = os.path.join(save_dir, f'fold{str(fold)}.csv')
                start_time = time.time()
                ...
                results_pred_list.append(result['pred'])
                results_true = result['true']
            pred_arr = np.array(results_pred_list)
            final_pred = [np.bincount(p).argmax() for p in pred_arr.T]
            cls_report = classification_report(
                results_true,
                final_pred,
                output_dict=True)
            ...
            # 保存分类报告为CSV
            report_save_path = os.path.join(save_dir, f'ensemble_report.csv')
            report_csv_file = pd.DataFrame(cls_report)
            report_csv_file.to_csv(report_save_path)
