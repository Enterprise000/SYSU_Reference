...
# 所有生成器模型的基类。生成器的作用是根据随机噪声或潜在变量生成与真实数据分布相似的数据（例如图像）
class Generator(nn.Module):
    ...
    # 这是一个生成数据的函数，通常在推理时使用。它从一个标准正态分布中生成噪声向量，并返回生成的噪声样本，用于输入到生成器中。
...
# 是所有判别器模型的基类。判别器的作用是区分生成的数据与真实数据。
...
# 是一个实现深度卷积生成对抗网络（DCGAN）生成器的类，这个生成器的设计是将一个随机噪声向量（编码）作为输入，并将其转换为一张图像。
...
# 是一个深度卷积生成对抗网络（DCGAN）判别器，用于判别输入图像是“真实的”还是“生成的”。
