...
# 该类将图像（以NumPy数组的形式）转换为与OpenCV兼容的格式。
class ToCVImage:
...
# 该变换随机从图像中裁剪一个区域，裁剪区域的纵横比和面积会在指定的范围内随机选择。
# 然后，裁剪后的区域会被调整为固定大小（例如224x224）。
...
# 将 OpenCV 图像（NumPy 数组，形状为 (h, w, c)，像素值范围为 0 到 255）转换为 PyTorch 张量(c, h, w) 的形状
class ToTensor:
...
#将 PyTorch 张量（形状为 (c, h, w)，像素值范围为 [0, 1]）
#转换回OpenCV 图像（NumPy 数组，形状为 (h, w, c)，像素值范围为 0 到 255）。
class ToArray:
